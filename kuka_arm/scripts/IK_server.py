	...
	theta5_prev = 0
	    ...
	    # Extract end-effector position and orientation from request
 	    ...
 	    R0_EE = rotz(yaw) * roty(pitch) * rotx(roll) 		      
            ...
            # Joint 2
            a = sqrt(d_4**2 + a_3**2) # virtual straight link (03 -> WC)
  
            s1 = wz - d_1 
            s2 = sqrt(wx**2 + wy**2) - a_1 
            b = sqrt(s2**2 + s1**2)

            c = a_2 # distance joint 2 -> 3

            alp1 = atan2(s1,s2) # aux. angle triangle 1
            A = acos((b**2 + c**2 - a**2)/(2*b*c)) # aux. angles triangle 2
